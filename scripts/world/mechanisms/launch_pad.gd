...
var pos := Vector2.ZERO          # 板中心(世界坐标)
var launch_vec := Vector2(0, -1400)   # 发射速度(px/s)
var _cooldown := 0.0
...
func _draw() -> void:
	...
	draw_rect(Rect2(-70.0, -14.0, 140.0, 4.0), Color(Ui.RED, 0.8))
	var dir := launch_vec.normalized()
	var arrow := clampf(launch_vec.length() / 280.0, 26.0, 64.0)
	var tip := dir * arrow
	var n := Vector2(-dir.y, dir.x)
	draw_line(Vector2(0, -18), tip, Color(Ui.PAPER, 0.85), 3.0)
	draw_colored_polygon(PackedVector2Array([
		tip, tip - dir * 14.0 + n * 9.0, tip - dir * 14.0 - n * 9.0]),
		Color(Ui.PAPER, 0.85))
	if _cooldown > 0.0:
		draw_rect(Rect2(-70.0, -14.0, 140.0, 28.0), Color(Ui.PAPER, 0.2))
