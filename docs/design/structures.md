...
4. **组件语义统一**(v0.13 立项;v0.44 组件语义 v4 拍板,层概念退役):一切构件与
   平台一样携带语义组 `faces / who / tags` + 组件编号 `id`
   (定义见 levels.md §7:faces 四值,who 为几何体
   集合、空 = 全员共享)——构件不再有"碰撞特权",门 / 板 / 开关与
   地形同一套通行规则;运行时编译共用该模型(levels.md §7 / Comp)。
...
| 构件 | 概念 | 立项理由 | 依赖 |
|---|---|---|---|
| **开关门 LeverGate**(v0.13 新立 · v0.15 一门多开关) | 场内踩踏开关与门板成对;踩下 ↔ 门板 faces 在 full/none 间切换,门板虚化态保留 8% 亮度线框;**一门可配多只开关(任一踩住即开)**,开关与门之间画地面连线标出归属 | 动态组件首版成员;合作分工新语言——一人踩门一人过;多开关 = **气闸式互让**(门两侧各一只:先过者踩住对侧接留守者),序章 05 机关·合拍首用 | **组件语义 v4**(运行时碰撞位切换,levels.md §7) |
| **限时桥 TimedBridge**(v0.13 新立) | 周期性实心 ↔ 虚化(on/off 各 ≥1s 保证可读),虚化期无碰撞、降透明度 | 动态组件首版成员;给"出手时机"再添一个全局钟;可订阅节拍时钟踩在拍点上(audio.md §5) | **组件语义 v4**(同上);节拍联动依赖 BGM 序列器 |
...
| **充电桩 ChargingPile**(v0.18 立项) | 双子专属踏板;伍任一半体踩住期间,磁感线边界特性**整体失效**(不再阻挡任何几何体),离开即恢复;桩体常亮紫色,磁界解除瞬间给全局可视反馈 | 给「隔」一个全局开关——磁界从常态墙变成可谈判的资源;触发归属 who={4} ≠ 效果范围(全局),是 v4 归属模型的范例件(levels.md §7) | 组件语义 v4(who 集合)+ 磁界(已实装) |
| 单向闸 OneWay | 仅单侧可穿的平台薄板 | 竖直关卡防回头 | 组件语义 v4 下 = faces=top 语义,不再是独立构件 |
