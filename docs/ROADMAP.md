...
> **状态注记(2026-09-14,Phase 7 收口)**:本文为**历史规划存档**
> (v0.16–v0.22 时代)——§1 组件化图层系统已随 v0.39.0 地图分层退役、
> §2 同屏双人 N1 / §3 跨设备 N0-N2 已实装、§7 机制完善期已转向。
> **现行计划唯一出处 = docs/REFACTOR.md**(§五 Phase/§十 收口排序/
> §十一 门禁台账)与 story.md §1.5 七幕主纲;本文不再维护。
...
| N2 同网直连 | `lan_beacon` 发现 + 房间 UI + ENet 直连 + 同步规格 | **已实装(v0.29.0)**:房间流程页 + 事件漏斗 + 绑定集内切换;PC↔Android 真机联测进行中;`--nettest` 通过 |
