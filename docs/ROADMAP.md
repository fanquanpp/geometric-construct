...
| V2 伍·界/边 | 双体 def + 三角绘制 + 磁力边界系统(BOUNDARY_BIT,逆/伍豁免)+ 同念共动 + 档案页双体卡 + 试水关 `levels/pair_trial.json`(--leveljson 钩子,兼编辑器数据契约前置) | **已实装(磁界谜题正剧关卡随第二幕)** | tourshot/trialshot ✓ + 手测检查点(用户) |
