...
- ⏳ 坐标化辅助设计(levels.md §8.2–8.4,2026-09-10 定稿待实装):
  zones 分区坐标系(分区挂层 + HUD 分区读数 + tours/hints 分区引用)、
  `--debug-grid` 叠加层(组件 id·层标注 + 受控者层签名)、
  `--gridcheck` headless 校验器(吸附 / 净空 / 行程 / 越界 / 伍门可达);
  Dimensions 量尺表(§8.1)已定稿并按表人工执行
