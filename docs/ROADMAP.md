...
> 当前为**机制完善期**(v0.17 起:演出关卡清空,唯一在演机制试炼场;
> 分层语义 v3 已实装,档案几何已扩为五页签全面档案库(v0.19 起步,v0.21.2 增键位指南)。
...
| 同屏双人(同设备) | **已实装(v0.28.0,net.md §3)** | 输入抽象(输入槽,N0)✓ | modes / entities / ui |
...
> **v0.18 拍板并实装(2026-09-10,v0.18.0)**:分层语义 v3(levels.md §7.10)
> ——八层定值 × who 集合归属 × 组件编号 id × 高亮/暗度三档;
> `lanes` / `far` 已废弃(旧档读取兼容保留在 `Comp.normalize`)。
...
| N1 同屏双人 | 双槽位绑定 / 镜头缩放框 / roster 双高亮 / 双人出生点 | **已实装(v0.28.0)**:`--dualtest` 五链路 PASS;双人试炼场通关待真机走查(关卡清空期,"第一幕任一关"折算为试炼场;桌面 + 触屏各验一轮) |
...
- ✅ 分层语义 v3(v0.18.0,2026-09-10 七项拍板并实装,levels.md §7.10):
  八层定值 L1–L8(实体性写入层表)+ `who` **集合**归属 + 组件编号 `id`;
  `lanes` / `far` 废弃;高亮/暗度三档泛化到全部机关物(FocusDriver);
  碰撞位上限守卫(修磁界位重叠隐患);layer_check 重建为 v3 套件 PASS;
  顺带修复逆的磁界穿透旗标从未置位的存量 bug
- ✅ 档案几何深度升级(v0.19.0,2026-09-10):GeometryPanel → ArchivePanel
  四页签(几何体档案 / 建筑物图鉴 / 机关图鉴 / 剧情回顾);aseprite 图鉴
  插图管线(统一 200×200 画布,`assets/archive/` 唯一入引擎目录,
  art-style.md §6.1)+ 动态精灵(终点门吸入 3 帧 / 限时桥 1s 循环 /
  传送对规划条目 3 帧)+ 手柄(十字键翻页 / LB·RB 切页 / B 返回)+
  安全区自适应版面;条目纯数据表 `ArchiveData`,扩容零代码
- ⏳ 充电桩机关(structures.md §5 立项):伍任一半体踩住 → 磁力边界
  整体失效,离开恢复;v3 归属模型已就位,随机制打磨期实装
- ✅ 坐标化辅助设计(v0.20.0,levels.md §8):zones 分区坐标系
  (六分区数据 + 网格分区标注 + HUD `分区 · x,y` 读数)、
  `--debug-grid` 叠加层(组件 `id·层` 标注,PC 参数 + 设置面板开关
  双端一致)、`--gridcheck` headless 校验器(吸附 / 净空 / 行程 /
  越界 / 伍门可达 / 共面接缝六类,试炼场全绿);Dimensions 量尺表
  (§8.1)生效并作为 gridcheck 的规则来源
