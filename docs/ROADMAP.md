...
- **瓦片渲染后端 TileRenderer**(双管线收敛,art-style.md §6.7):与
  `LaneRenderer` 并列消费同一 Comp 语义——按 Rect2 暴露位取片 +
  内部平铺 + 边缘裁切,碰撞与 lane / who modulate 语义全引擎侧;
  编辑器产物强制瓦片路径,官方 10 关维持 `_draw()`,此后做"统一替换
  适配"单次发版决策(`--tourshot` 全关 A/B 截图对比后定夺)。
...
- **LAN 发现**:`PacketPeerUDP` 受限广播 `255.255.255.255` +
  ...
  复用编辑器 M3 二维码基建)三重兜底;发现应答带版本 + 关卡数据哈希
...
- **测试钩子**:autotest AI 抽成独立类,编辑器"可通关试探"与
  联机主机模拟都复用。
- **UiRouter 页面栈**(ui-flow.md §7,触发线 = 编辑器 / 客席剧目 /
