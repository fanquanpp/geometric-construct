...
- **SVG(UI 图标 / 角色徽标)**:在用 18 枚(v0.46.0 重绘)直改
  `assets/svg/`;全套 69 枚再生源 = `tools/redraw/spec_icons.py`
  (改规格后跑 tools/redraw/build.py 重新导出)。SVG 渲染器(ThorVG)
  **不支持 `<text>`**,文字一律用折线字形。
...
- [ ] 新增 SVG 走 `tools/redraw/spec_icons.py` 管线或在用件直改
	  `assets/svg/`;新增图鉴插图直接更新 `assets/archive/` PNG
	  (v0.46.0 重绘版为孤本,地图零贴图纪律不破)
