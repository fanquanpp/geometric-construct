...
## 未发布(v0.18.1 设计定稿,未实装)

> 2026-09-10 定稿:**坐标化辅助设计**(levels.md §8 升级)。定位网格
> 从背景装饰升格为分层地图设计的辅助工具链,与 §7.10 分层体系强关联。

### 新增(设计)
- **Dimensions 坐标量尺表**(§8.1,已定稿即生效):全游戏空间尺寸只
  允许从玩家物理量推导——头顶净空 ≥1.0 格、走道 ≥1.2 格、跨坑 2.6/
  4.6/5.5 格分档、坡度 ≤37°、台阶 ≤0.9 格;参照 Valve《Dimensions》
  量尺表与 Celeste"以体格定格单位"。
- **zones 分区坐标系**(§8.2,待实装):`def.zones = [{rect, name,
  layer?}]`,分区作为组件同样挂层;分区名标注在网格上,HUD 读数扩展
  `分区名 · x,y`(跨界防抖);tours / hints / 日志改用分区名引用,
  消除布局迭代时坐标引用失配(v3→v4 重标教训)。
- **`--debug-grid` 叠加层**(§8.3,待实装):组件左上格点标注
  `id·层`,who 组件附层色点;HUD 读数附加受控者当前层签名。
- **`--gridcheck` headless 校验器**(§8.4,待实装):吸附 / 净空
  (按 who 逐几何体裁定)/ mover 行程 / 越界 / 伍门双体可达五类检查,
  违规非零退出,接入 autotest 与发版钩子。

### 变更(设计)
- 网格定位从"三位一体"扩为"四位一体":坐标系 / 量尺 / 校验基准 /
  分区索引;既有三级 LOD 与视差解耦保持不变(§8.5)。
