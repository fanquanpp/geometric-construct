...
> 2026-09-10 用户七项拍板:**分层语义 v3**。全文见 levels.md §7.10;
> 实装清单同节,动代码时一次完成。
...
- **高亮 / 暗度三档**:专属件(who 含受控者)按受控几何体色提亮 +
  描边脉冲,共享件常亮,无关件幽灵暗度;泛化到全部机关物
  (v2 只有 platforms 参与虚化,机关物"无碰撞却画成实心"一并修复)。
- **充电桩机关立项**(structures.md §5 / characters.md §5):伍任一半体
  踩住 → 磁力边界整体失效,不再阻挡任何几何体,离开恢复。

### 变更(设计)
- 碰撞签名键从 midset 改为 `(layer, who 集合)`,构建期动态分配不变;
  渲染谓词与碰撞谓词合并为唯一实体化函数(消灭 `_solid_for` / `_midset`
  两份拷贝)。
- 置换(逆)落点语义过滤(§7.5)随 v3 谓词一并实装(此前仅存在于文档)。

### 移除(设计)
- `lanes`(逐几何体层级)/ `far`(不适用沉降档)废弃——一组件一层,
  分层差异拆两个组件表达;`Comp.combo_key` 死代码一并清除。
