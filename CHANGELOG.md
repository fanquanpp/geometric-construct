...
> ③编辑器仍有无图形节点 ④geo_index 越界/未绑定排查 ⑤只用引擎原生与 aseprite。**

### 新增
- **UI 图标 aseprite 管线**:`tools/gen_icons.lua` → `assets/ui/icons.png`
  图集(64px 网格 × 18 格:角色徽标 5 / 键帽折线字形 7 / check·play·
  recall·pause 及 on 态 6);`Ui.icon` 切图集 AtlasTexture(键名去 .svg,
  全部调用点随改),`assets/svg/` 18 枚与 `Godot_logo_icon.svg` 整体退役
  (boot 引擎署名改纯文字,CC BY 出处保留)。
- **档案插图重构**:`tools/gen_archive.lua` → 43 帧同名覆盖(15 建筑 +
  5 几何肖像 + 23 机关帧)——严格 §6.1 构成纪律(body 平涂 + 内缘自阴影 /
  panel 顶受光 / edge 纸白顶缘 / motif 白纹墨错位;几何肖像不画投影);
  机关帧 = 关卡内正典帧,图鉴即实机。v0.46 重绘包的柔光斜面风格
  (偏离 §2 禁渐变/禁柔影)整体替换。
- **图块集 v2**:`gen_tiles.lua` 纸白顶缘提亮(EDGE 全系纸白调,§4/§9
  深底亮缘)+ 邻接族加顶受光带(§6.1 panel);坐标/碰撞契约零改动;
  生成器改自建 1600×1400 画布(根治旧 40 格工作文件覆盖事故)。
- **编辑器可见性补齐**:MagBoundary @tool 占位预览(hint_span 虚折线)、
  `SpawnMarker` 出生旗场景(几何体色旗面 + 红基座,新关可拖摆)、
  ramp 未配置虚线斜面占位。

### 修复
- **MagBoundary 双 `_draw` 重名解析错误**(占位预览与运行时绘制未合并,
  连锁 27 关装载失败):合并为单函数,五门禁复跑全绿。
- `addons/addons/godot_ai/` 嵌套重复目录(插件误装)刷 UID duplicate
  警告——已删;27 关编辑器全量警告普查后归零。
- geo_index 普查:全部关卡 41 处 door 值域 [0,4] 零越界;脚本绑定
  零缺失(用户所见告警 = 未配置机关的配置警告族,均已配可视占位与
  黄叹号指引)。

## v0.47.1(2026-09-16 · 画册全量重截 + 旧生成器清退)

> **UI 图册按自身纪律全量重截(v0.46 重绘后)+ 隐患工具清退 + MCP 链路诊断结论**。

### 变更
- **docs/ui 画册全量重截**(纪律:改完必须重截对应屏):菜单/剧目/档案
  五页签(geo·keys·bld·mech·story)+ 剧情目录新页 / 设置 / 开场卡 /
  双人两屏 / 房间四屏,共 16 屏换新;boot 两屏未受影响沿用。
  页签结构换代入册:图鉴页已拆建筑/机关两页(ui_11 → ui_11 + ui_11b),
  新增剧情目录页(ui_12b)。
- **tools/build_native_levels.py + act2_src/(34 JSON)清退**:二~五幕
  重摆已由 restyle 工具完成,旧生成器重跑会覆盖重摆成果(隐患);
  原始布局档见 git 历史。

### 修复
- MCP 编辑器链路诊断收口:本环境 godot-ai 服务端由 CLI 以 stdio 拉起,
  编辑器插件的 WebSocket 桥无法回连(需服务端以 streamable-http 启动)
  ——编辑器内自动验证暂不可用,视觉验收走 CLI 分镜,结论入记忆。

## v0.47.0(2026-09-16 · 磁界复活 + 二~五幕图块重摆)

> **伍·界/边磁力机制回归(场景壳化)+ 二~五幕 19 场邻接变体重摆 + 幕差分装饰**
> (用户拍板:①磁界重构复活 ②字体保持不缩 ③二~五幕由 AI 重摆 ④推送提交)。

### 新增
- **MagBoundary 复活**(v2 速度投影版,根治扫掠推挤):场景壳化
  `scenes/world/mechanisms/mag_boundary.tscn`;`native_level.gd` 装配修复——
  名册含伍时 `create_character` 返回值接线 **partner 互引 + 磁界端点**
  (修复现役 10 个伍场双体契约缺失的实锤 bug,act1/s06、act2 全部、
  act4/s05、act5/s03-04 自动生效)。
- **移动平台行程预告线**并入 mover `_draw`(接管已清退的 MoverTrack;
  行程线 + 端点红刻度,编辑器内同绘,所见即行程)。
- `tools/restyle_native_acts.gd`:二~五幕图块重摆工具(确定性、幂等、
  跳过用户重摆中的 act2/s01)。

### 变更
- **二~五幕 19 场图块重摆**:Solid 素板 `(0,0)` 全量升级为石板 16 邻接
  变体(2424 格;全族整方碰撞,物理逐格等价,levels.md §0 契约);
  Decor 增幕差分点缀(暗板 / 红刻 ≤1 每场 / 幕色焦点框按幕取色,
  守「每屏红 ≤3」纪律)。
- 视觉验收:四幕抽样分镜实拍(磁界张成 / 变体缘线 / 装饰在位);
  MCP 编辑器会话未能建立(插件传输待查),降级 CLI 分镜,如实记录。

### 修复
- **新 class_name 未注册时批量重存场景会剥掉根脚本与导出属性**
  (restyle 首跑实锤:MagBoundary 无类缓存 → native_level.gd 编译失败 →
  pack 出无脚本场景;git 恢复 19 场原件后于 import 完成后重跑修复;
  工具已加防御性补挂)。教训入册:批量重存场景的工具,先 --import。
...
- **图块集换代 224 格**(Aseprite 全量绘制;`tools/gen_tiles.lua`
  ...
  **过渡件族 32**(变厚·薄↔整 / 凸凹节点 / 半高↔全高 / 削角 45°
  墙柱×4 / 材质交界×4 / 柱头柱脚 / 坡脚跑平×2 / 双层桥·天桥·
  双层天花 / 踢脚·顶裙·腰线·分层·女儿墙·檐口·嵌筋·百叶带)。
  **逐格图像识别审计勘误五处**:(12,0) 取景框与 (3,0) 角刻同质化
  → 括弧加大加粗+中心点;(0,2)(1,2)(11,2)(14,2) INK3-on-DARKV
  细节过隐 → 提为 DIM;(12,2) 山脊纸白线补齐四边;(14,4)(15,4)
  亮面板坡缘色按底色取色;(1,11) 下箭头杆头接续。
  **TileSet 同步重建**(`native_tileset.tres`):224 图位逐格碰撞
  (整方 / 顶缘 24px·12px One Way / 逆天花板底缘反向 One Way /
  半高·柱·门框·阶台局部多边形 / 坡件三角与三级阶梯多边形 /
  削角五边形 / 过渡件分段多边形 / 装饰无碰撞);兼容契约
  (0,0)(1,0)(2,0)(0,1)(1,1) 五格不变。
...
### 新增
- `story/act2.ks`「界与边」三拍:第五刻度落地成双 → 界 / 边自报家门(量天 / 量地成对互答)→ 边界之问收束。
- `story/act3.ks`「分岔」:四条岔路只认一个形状,"独自一人时,我还算什么"拆成四份。
- `story/act4.ks`「蜕变」:巨构第一次给错的路,四份不一样的能力课答案。
- `story/act5.ks`「刻度的真相」:最深处的密刻,代价第一次可见,铺向五门。
- 回廊目录 STORIES 登记 act2~act5(回廊可回顾);存档旗标 `seen_act2..5`(save_manager 写/记/读三侧)。

### 变更
- `epilogue.ks` 重写为**正式落幕幕**(五门归位版):五形蜕变总结 → 回声拍(第一幕旧话重现,正典台词保留)→ 更深空白两形一闪(陆 / 柒,留白不解释)→ "落幕。全员归位"。旧四门版为第一幕终演占位,已被替换。
- `game_flow` 开演剧触发泛化:首进任意幕首场播该幕 `act{N}.ks`(旗标 story_seen/note_story 按 kind 分发),不再硬编码第一幕。
- story.md §4(落幕幕档案)+ §4b2(四段开演剧档案);scenes.md 场次总表 8 → 12 场;version PATCH 1(v0.44.1)。
...
> **组件语义 v4(层概念整体退役)+ 测试关清退 + 正戏五幕 26 关全量实装 + 通关流转出生修复**。

### 修复
- **通关切下一关「无法诞生」P0**:`game_flow.clear_level` 的 `queue_free` 延迟释放窗口里,旧 LevelRoot `_init` 连接的 `character_created` 同帧抢挂新玩家——玩家随旧根被释放,下一关 `players` 为空(v0.38.2 流转域迁移把 v0.31 的 `free()` 教训回退所致)。恢复 `free()` 立即释放;新增 `tests/flow_check` 通关流转门禁(通关 → 自动切关 → 受控体出生点断言,退出码即结果)。
- 伍双体出生两处:①spawns 按几何体下标索引,作关脚本手写占位与自动补位叠加使双体字典落位下标错位(伍落原点,界嵌天花被弹出越顶摔碎循环);②出生 y 嵌入天花样——五幕伍关 `a` 点统一 270(体半高 15 + 天花底 200 + 余量)。

### 移除(废弃退役内容清账)
- **层概念连根拔**(用户令:八层设计系统彻底删除,全部 Godot 原生):`Comp` 的 `layer` 字段、`LAYER_*` 常量、`norm_layer` / `is_solid_layer` 全删;装饰 = `faces:"none"`;碰撞签名 = who 集合(v4);HUD 坐标读数去 L 编号(层签名 → 共享/专属);`--debug-grid` 去 id·层标注;zones 去 layer 字段;全部关卡 JSON 去 layer 字段(act1 19 件装饰转 `faces:none`)。
- 测试关卡清退:`trial_v5`(机制试炼场)与 `pair_trial`(伍试水)整删,幕 0 注册表移除,正戏五幕直接开场;tourshot 旧 v4 手排节拍表退役,改**数据驱动**(出生点 + 检查点 + 实体平台顶 x 均布采样,跳过出口门 ±120 防误触 WIN,伍传送地面半体)。
- `layer_check.gd` → **`comp_check.gd`**(who×faces 语义套件:mask 编译 / who 碰撞 / 单向面 / 开关门 / 限时桥 / 位上限 / 画序带 Decor0<Solid1<玩家5);`grid_check` / `reach_check` 去层谓词;AGENTS 验收基线行同步。

### 新增(正戏五幕全量,bible 卷五幕问落地)
- **第二幕「界与边」六场**:会合 / 过往的痕迹(第七形剪影)/ 隔开与保护 / 镜像双塔 / 非对称的缝 / 五门并立(第五刻度收拢)。
- **第三幕「分岔」五场**:疾·独行 / 跃·高台 / 逆·两面 / 圆·长坡(四单章正剧化)/ 分岔·路口。
- **第四幕「蜕变」五场**:疾·学会停(窄台禁冲)/ 跃·为自己折(深谷受接)/ 逆·落地(全关零天花)/ 圆·回头(去程回程)/ 蜕变·终场。
- **第五幕「刻度的真相」四场**:刻度长廊 / 代价(限时桥+推箱)/ 第四层真相(全机关合流)/ 落幕·五门归位(终关通关接尾声)。
- 作关器 `tools/author_acts.py`(20 关 JSON 规格源:who×faces v4,改关重跑);关卡数据表五幕 26 关(levels 0–25)。

### 变更
- `level_data.gd` ACTS 重排(5 幕);`game_flow` 幕归属 / 开演剧触发随幕表泛化(第一幕=首幕)。
- 关卡作关纪律实锤:逆 / 圆 `can_jump=false` —— 其关地面必须连续(缝不可跳),上行踏步链 dy≤150、平缝≤250;伍门区 = 双台同高 680 + 门楣挂点(底 590)+ 门区 ±86 双落点。
...
### UI 全量审计(两轮 · 14 屏)
- 第一轮:标题菜单 / 剧目二级 / 设置 / ROOM 三页(认领/模式/主机)/ 档案五页签(panelshot 系)——**全部 PASS**:Primary 红实心 / 双人橙描边 / 次级描边语义色统一;满幅排版与红块题头同语言;认领态与满员禁用逻辑正确。
- 第二轮:肉鸽五页(选主角/选路/词条/结算/状态)+ 局内 trial 三镜——RERUN 独立语言(红题头/角色色卡/残段显示)与 LayerVisual 局内渲染零漂移。
- **结论:零风格漂移,无需修复**——历次域拆分批次均保持 ui-flow 规范。

### 真机迭代(K60 Ultra × 2 轮)
- 轮 1:开局链(开始/继续 → 开场卡跳过 → 局内)——**LayerVisual 真机渲染首验 PASS**(石板亮线/加速门/提示牌/网格/坐标读数含层签名);开场卡四角红刻与跳过钮正常。
- 轮 2:暂停菜单(红题头/六钮/世界冻结压暗)PASS。
- logcat 零引擎错误。

### 发布
- GitHub Release v0.42.0(附 Android debug apk)。
...
### 新增
- **tools/zato_stylometry.py**:VN 脚本文体量化工具(解析双语 rpy,剥英文灰字与标记;联网核对——VN 脚本形式文体学尚无已发表研究,本工具为原创方法)。
- **zato_reading.md §4 八维剖面**:九章 × 台词量/对白旁白比/均长/断句率/停顿密度/cps0 全表。量化发现四条——①缓冲章=**载体切换+时间减速**(ep2d3 描/对 2.87 全篇唯一旁白主导 + 停顿密度 716/千行=其它章 10-200 倍);②**对白均长是节奏杠杆**(旁白均长稳定 9.6-13.1,对白 4.4→7.3 随张力拉长);③高潮三共振(ep2d7 对白最长+停顿最密+禁跳字最多);④描述对白比/停顿密度/对白均长三键即可刻画节奏型,可移植为 Konado .ks 量化审稿指标。
- **bible-zato.md §二 量化注记**:信息滴灌高潮期从"藏"切"强制慢读";锯齿缓冲场判定=描述对白比反转+停顿抬升(非单纯行数);声线表高潮放宽至 3 行为显性张力信号(候选)。
- **肉鸽×联机零行为预埋(roguelike.md §7 首两项)**:RunState.owner_slot(单机恒 0)+ NetSession EV_RUN_MOD/TICK/PATH 三事件位常量预留。

### 门禁
- check-only 两文件绿 / MODCHECK PASS / NETTEST ALL PASS(预埋零行为验证)。
